...
from .configuration_gemma import MyGemmaConfig
...
class GemmaAttention(nn.Module):
    ...
    # Ignore copy
    def __init__(self, config: MyGemmaConfig, layer_idx: Optional[int] = None):
        super().__init__()
        self.config = config
...
class GemmaDecoderLayer(nn.Module):
    def __init__(self, config: MyGemmaConfig, layer_idx: int):
        super().__init__()
        self.hidden_size = config.hidden_size
        ...
        self.adaLN_modulation = nn.Linear(config.hidden_size, 6 * config.hidden_size, bias=True)
    ...
    def modulate_with_mask(self, x: torch.Tensor, shift: torch.Tensor, scale: torch.Tensor,
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states = self.modulate(hidden_states, shift_msa, scale_msa)
        ...
        hidden_states = self.modulate(hidden_states, shift_mlp, scale_mlp)
...
class GemmaModel(GemmaPreTrainedModel):
    ...
    def __init__(self, config: MyGemmaConfig):
        super().__init__(config)
        self.padding_idx = config.pad_token_id
        self.vocab_size = config.vocab_size
        self.time_embedder = TimestepEmbedder(config.hidden_size)
        self.embed_tokens = nn.Embedding(config.vocab_size, config.hidden_size, self.padding_idx)   # vocab_size 256,000+8,192=264192
    ...
    def _update_causal_mask(self, attention_mask, input_tensor, cache_position):
        ...
        target_length = seq_len  # no causal restriction
        causal_mask = torch.zeros((batch_size, 1, seq_len, target_length), dtype=dtype, device=device)

        if attention_mask is not None:
            # (B, 1, 1, L)
            expanded_mask = attention_mask[:, None, None, :].to(dtype=dtype)
            #expanded_mask = (1.0 - expanded_mask) * torch.finfo(dtype).min  # set padding to -inf
            #causal_mask[:, :, :, :attention_mask.shape[-1]] = expanded_mask
            padding_mask = expanded_mask == 0  # bool mask
            causal_mask = causal_mask.masked_fill(padding_mask, torch.finfo(dtype).min)
...
class GemmaForCausalLM(GemmaPreTrainedModel):
    ...
    def __init__(self, config):
        ...
        self.vocab_size = config.vocab_size
        #print(f'new vocab_size:{config.vocab_size}')
    ...
    def forward(
        ...
        #print(self.lm_head)
        mllm_outputs = self.lm_head(hidden_states)
        mllm_outputs = mllm_outputs.float()

        img_start_ids = 2
        img_end_ids = img_start_ids + 1024
        txt_start_ids = input_ids.shape[-1] - 256 - 2
        txt_end_ids = input_ids.shape[-1] - 2
        logits_img, logits_txt = mllm_outputs[:, img_start_ids:img_end_ids, :], mllm_outputs[:, txt_start_ids:txt_end_ids, :]
        logits = torch.cat([logits_txt, logits_img], dim=1)

        # sampling_kwargs = {'temperature': 0.99, 'top_k': 4096, 'top_p': 0.96, 'sample_logits': True}
        # logits, _ = self.sample(logits, **sampling_kwargs)
        # print(logits.shape)

        if not return_dict:
            predict = (logits,) + outputs[1:]
            return predict
