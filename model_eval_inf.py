...
def on_validation_epoch_start(self):
    ...
    if self.ema is not None and not self.config.trainer.use_custom_ema:
        ...
        print(" [WARNING] USING EMA IN on_validation_epoch_start - THIS MIGHT RESET LOADED WEIGHTS ".center(100, "!"))
...
def sample(self, return_input_ids=False, epoch=None, image_save_postfix=None, **kwargs):

    text_only = kwargs.get("text_only", False)
    kwargs.pop("text_only", None)
    #print('text_only:',text_only)
    #txt_tokens, img_tokens = self._sample(text_only=text_only, epoch=epoch, image_save_postfix=image_save_postfix, **kwargs)
    out = self._sample(text_only=text_only, epoch=epoch, image_save_postfix=image_save_postfix, **kwargs)

    #if img_tokens is not None:
    #    img_pred = decode_latents(self.config, self.image_tokenizer, img_tokens)
    #else:
    #    img_pred = None
    #if txt_tokens is not None:
    #    txt_pred = self.text_tokenizer.decode(txt_tokens[0].long().tolist()).replace('<unk>', '')
...
@torch.no_grad()
def validation_step(self, batch, batch_idx, step, name=None):
    ...
    #):
    #if step > 1:
    self.sample_masking(batch=batch, batch_idx=batch_idx, epoch=step, name=name)
...
def on_validation_epoch_cleanup(self):
    empty_device_cache()
    print("on_validation_epoch_cleanup finished")
...
def _ddpm_forward(self, x, t, sigma_t, batch, x0=None, x0_unmask=None, force_cfg=None, **kwargs):
    _w = None
...
def sample_masking(self, batch, batch_idx, epoch, name=None):
    ...
    if name is not None:
        ...
        mask_name = random.choice(['mask_all','mask_text_only', 'mask_image_only', 'only_uniform_mask', 'conditional_mask'])

    if mask_name == "mask_all":
        _move_chance = 1.0
        ...
        xt = x0.clone()
        xt[:, :256] = self.mask_index
    elif mask_name == 'mask_image_only':
        xt = x0.clone()
        xt[:, 256:] = self.mask_index
    elif mask_name == 'conditional_mask':
        prompt_idx = batch["prompt_idx"].squeeze().item()
        #prompt_idx = 200
        xt = x0.clone()
        #print(f'xo begin:{x0}')
        #print(f'prompt index:{prompt_idx}')
        xt[:, prompt_idx:] = self.mask_index
        #print(f'xo begin:{xt}')
    else:
        move_indices = torch.rand(*x0.shape, device=x0.device) < move_chance
        xt = torch.where(move_indices, self.mask_index, x0)
    ...
    #print(xt.shape, xt_unmasked.shape, _x_modality)
    self.sample(x0=xt, x0_unmask=xt_unmasked, sample_modality=_x_modality, batch=batch, epoch=epoch, image_save_postfix=mask_name)
...
@torch.inference_mode()
def _ddpm_caching_update(self, x, t, dt, p_x0=None, x0=None, x0_unmask=None, modality=None, batch=None,**kwargs):
    ...
    if p_x0 is None:
        ...
        _sigma = sigma_t
        #print(batch['boi'].shape)
        p_x0 = self._ddpm_forward(x, t, _sigma, batch=batch, x0=x0, x0_unmask=x0_unmask,  **kwargs)
        nfe_cnt += 1
    assert move_chance_t.ndim == p_x0.ndim
...
@try_except(clear_cuda_cache=True)
@torch.inference_mode()
def _sample(
        self,
        num_steps=None,
        ...
        epoch=None,
        image_save_postfix=None,
        **kwargs,
):
    batch_size_per_gpu = 1
    kwargs['sample_ids'] = sample_ids
    return_nfe = False
    is_sample_masking = False
    allow_interleaved_conditional = False
    nfe_cnt = 0
    self.sampler = "maskgit"
    assert batch_size_per_gpu > 0
    if num_steps is None:
        num_steps = self.config.sampling.steps

    decode_kwargs = dict(sample_modality=sample_modality, return_raw_data=return_raw_data,
                         is_sample_masking=is_sample_masking)
    ...
    if num_steps > (~x0_unmask).sum(dim=-1).min():
        rprint(
            f"num_steps {num_steps} > sequence length {(~x0_unmask).sum(dim=-1).min()}, setting num_steps to sequence length")
    ...
    #if image_save_postfix == 'conditional_mask':
    #    print(x0_unmask, x0)
    ...
    #print(x.shape, x0.shape, x0_unmask.shape, batch['boi'].shape)
    #print(self.sampler)
    #self.sampler = "maskgit"
    #print(f'time:{num_steps}')
    for i in range(num_steps):
        #print(f'time:{i}')
        t = timesteps[i] * torch.ones(x.shape[0], 1, device=self.device,
                                      dtype=self.dtype if self.config.trainer.force_bf16_eval else torch.float32)

        if self.sampler == "maskgit":
            #print('yes')
            x, nfe_step_cnt = self._maskgit_update(x, t, dt, x0=x0, x0_unmask=x0_unmask, schedule=schedule, step=i,
                                                   batch=batch, **kwargs)
            x = torch.where(x0_unmask, x0, x)
        elif self.sampler == "maskgit_nucleus":
            x, nfe_step_cnt = self._maskgit_nucleus_update(x, t, dt, x0=x0, x0_unmask=x0_unmask, schedule=schedule,
                                                           step=i, batch=batch, **kwargs)
        ...
        #     nfe_step_cnt = 0
             ...
             p_x0_cache, x_next, nfe_step_cnt = self._ddpm_caching_update(x, t, dt, p_x0=p_x0_cache, x0=x0, x0_unmask=x0_unmask, batch=batch,**kwargs)
    ...
    if self.config.sampling.noise_removal:
        ...
        if self.sampler == "analytic":
            ...
            x = self.forward(x=x, sigma=unet_conditioning, batch=batch, **kwargs).argmax(dim=-1)

    if x0 is not None and x0_unmask is not None:
        x = torch.where(x0_unmask, x0, x)
    res = self.decode_sampling(x, text_only, epoch=epoch, image_save_postfix=image_save_postfix, batch=batch, **kwargs, **decode_kwargs)

    if return_nfe:
        return res, nfe_cnt
    return res

def decode_sampling(self, x, text_only, is_sample_masking=False, bypass_return_interleaved_modalities_split=False, epoch=None, image_save_postfix=None, batch=None, **kwargs):
    ret = self.decode_batch(x, text_only=text_only, **kwargs)
    #input_ids = batch["input_ids"]
    #gt_txt, gt_img = input_ids[:, :256], input_ids[:, 256:]
    # if getattr(self.config.eval, "visualize_sample", False):
    #gt_img = gt_img - 32004
    #gt_img = torch.clamp(gt_img, min=0, max=8191)
    #self.save_image_text_pair(gt_img, gt_txt, epoch=epoch, image_save_postfix='gt')
    #if getattr(self.config.eval, "visualize_sample", False):
    self.save_image_text_pair(ret[1], ret[0], epoch=epoch, image_save_postfix=image_save_postfix)
    return ret
...
def decode_batch(self,
    ...
    **kwargs
    ...
    if isinstance(sample, dict) or isinstance(sample, TensorDict):
        img, txt, size = sample['image'], sample['text'].to(torch.int64), sample['size']

        _, _, [_, _, vq_code] = self.image_tokenizer._vq_model.encode(img.to(self.device))
        img_tokens = vq_code.view(img.shape[0], -1)
        text_tokens = txt.to(self.device)
    else:
        ...
        if invalid_text_mask.sum() > 0:
            text_tokens[invalid_text_mask] = self.mask_index
        #print(text_tokens.shape, img_tokens.shape, img_tokens.min(), img_tokens.max())
        img_tokens = img_tokens - 32004
        img_tokens = torch.clamp(img_tokens, min=0, max=8191)
...
def safe_multinomial(self, p):
    p = p.clone()
    p[p != p] = 0.0          # 将 NaN 置为 0
    p[torch.isinf(p)] = 0.0  # 将 Inf 置为 0
    p[p < 0] = 0.0           # 将负数置为 0
...
# 用这个替代你原来的 multinomial
#pred_code = safe_multinomial(p_x0.view(-1, p_x0.shape[-1]), 1)[:, 0].view(p_x0.shape[:-1])


@torch.no_grad()
def _maskgit_update(self, x, t, dt, schedule=None, step=None, batch=None, **kwargs):
    sigma_t, _ = self.noise(t)
    sigma_s, _ = self.noise(t - dt)
    if sigma_t.ndim > 1:
        sigma_t = sigma_t.squeeze(-1)
    ...
    #copy_flag = (x != self.mask_index) | x0_unmask
    ...
    p_x0 = self._ddpm_forward(x, t, _sigma, batch, **kwargs)
    nfe_cnt += 1
    #print(p_x0.view(-1, p_x0.shape[-1])[0].min(), p_x0.view(-1, p_x0.shape[-1])[0].max())
    #pred_x0 = p_x0.view(-1, p_x0.shape[-1])
    
    p_x0[p_x0 != p_x0] = 0.0          # 将 NaN 置为 0
    p_x0[torch.isinf(p_x0)] = 0.0  # 将 Inf 置为 0
    p_x0[p_x0 < 0] = 0.0           # 将负数置为 0
    
    #row_sum = p_x0.sum(dim=-1, keepdim=True)
    #row_sum[row_sum == 0] = 1.0       # 避免除零
    #p_x0 = p_x0 / row_sum             # 归一化为概率
    probs = p_x0.view(-1, p_x0.shape[-1])  # (1280, vocab_size)

    # 找到哪些行是全 0
    row_sums = probs.sum(dim=-1)
    valid_rows = row_sums > 0

    # 先做个默认值（比如全 0 行就返回 0 这个 token）
    pred_code = torch.zeros(probs.size(0), dtype=torch.long, device=probs.device)

    # 对于合法行再采样
    if valid_rows.any():
        sampled = torch.multinomial(probs[valid_rows], 1)[:, 0]
        pred_code[valid_rows] = sampled

    # reshape 回去
    pred_code = pred_code.view(p_x0.shape[:-1])
    
    #pred_code = torch.multinomial(p_x0.view(-1, p_x0.shape[-1]), 1)[:, 0].view(p_x0.shape[:-1])
    #print(f'shape mismatch error:',p_x0.shape, pred_code.unsqueeze(-1).shape)
...
@torch.no_grad()
def _maskgit_nucleus_update(self, x, t, dt, schedule=None, step=None, batch=None, **kwargs):
    nfe_cnt = 0
...
def visualize_samples(self, batch, batch_idx, split='val'):
    ...
    gt_txt, gt_img = self.decode_batch(batch, text_only=False, sample_modality=batch.get("modality", None))

    if gt_img is not None:
        rec_img = decode_latents(self.config, self.image_tokenizer, gt_img)
        jpg_path = os.path.join(self.config.output_dir, 'save_data',"gt.jpg")
        os.makedirs(os.path.dirname(jpg_path), exist_ok=True)
        rec_img.save(jpg_path)

    txt_path = os.path.join(self.config.output_dir, 'save_data', "gt.txt")
    test_caption = self.text_tokenizer.decode(gt_txt[0].long().tolist()).replace('<unk>', '')
    with open(txt_path, "w") as f:
        f.write(test_caption)
    rprint(f"GT Captions: {self.text_tokenizer.decode(gt_txt[0].long().tolist()).replace('<unk>', '')}")
...
def save_image_text_pair(self, image_tensor, text_tensor, single_image_only=False, disable_img_save=False, epoch=None, image_save_postfix=None):
    """
    ...
    #date_folder = datetime.now().strftime("%Y-%m-%d")
    save_dir = os.path.join(self.config.trainer.output,'test')
    #save_dir.mkdir(exist_ok=True, parents=True)
    os.makedirs(save_dir, exist_ok=True)

    sample_img = decode_latents(self.config, self.image_tokenizer, image_tensor)
    sample_text = self.text_tokenizer.decode(text_tensor[0].long().tolist()).replace('<unk>', '')
    
    img_filename = f"image_{epoch}.png"
    img_path = os.path.join(save_dir, img_filename)
    gprint(Im(sample_img).save(img_path))

    text_filename = f"text_{epoch}.txt"
    text_path = os.path.join(save_dir, text_filename)
    with open(text_path, "w", encoding="utf-8") as f:
        f.write(sample_text)
    
    # eRth = os.path.join(save_dir, text_filename)
    return img_path
