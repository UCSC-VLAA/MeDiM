...
@torch.no_grad()
def validation_step(self, batch, batch_idx, step):
    ...
    #):
    if step > 1:
        self.sample_masking(batch=batch, batch_idx=batch_idx, epoch=step)
...
def sample_masking(self, batch, batch_idx, epoch):
...
def _ddpm_update_finetune_controlled_tweedie(self, x, t, dt, reward_model=None, repeats=10, sampling_step=None, **kwargs):
    ...
    if not hasattr(self, "reward_model"):
        from medim.tokenizers.laion_aesthetic_v2 import get_predictor_func
...
def get_hpsv2_score(
    ...
    from medim.tokenizers.hpsv2_img_score import score, initialize_model
...
def save_image_text_pair(self, image_tensor, text_tensor, single_image_only=False, disable_img_save=False, epoch=None, image_save_postfix=None):
    ...
    #date_folder = datetime.now().strftime("%Y-%m-%d")
    save_dir = os.path.join(self.config.output,'test')
    ...
    pair_img = []
    txt_image = create_text_image(text=sample_text, desired_width=self.config.data.resolution)
    pair_img.append(txt_image)
    pair_img.append(sample_img)
    paired_img = Im.concat_vertical(*pair_img).pil
    filename = f"pair_{get_rank()}_{epoch}"+ f"_{image_save_postfix}.png"
    save_path = os.path.join(save_dir, filename)
    gprint(Im(paired_img).save(save_path))
